...
for Mf_id = 1:1:length(MF_container)
    Rho_power=sqrt(Mf_id+20*(info_length/Mf_id));
...
%     PN_code=AGWN_Generator(length(find(OfdmSymbol_Pilot(1:end) ~= 0)));
    ...
    OfdmSymbol_Pilot=sqrt(Rho_power)*OfdmSymbol_Pilot;
	...
	for SNR=0:1:1
		...
		for ii=1:length(ChannelOutput2)-length(ChannelInput_Pilot);
			...
			for jj=1:length(ChannelInput_Pilot)
                Gamam_P_temp=Gamam_P_temp+ChannelInput_Pilot(1,jj)*conj(ChannelOutput2(1,jj+ii-1));
			end
		...
		argmax_before_P=abs((1+Rho)*real(Gamam_P)-Rho*real(Feim_P));
...
% 		Error_ratio_time(1,kk)=error_ratio_time/(FrameNum-2)
	end
	MSE_time
    if Mf_id==1
       picture_style='-o';
