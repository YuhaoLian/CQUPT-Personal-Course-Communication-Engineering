...
for SNR=0:1:10
    ...
    Gamam_P2=Gamam_P(1,1:end-Info_length);
    Gamam_P3=Gamam_P(1,Info_length+1:end);
    
    argmax2=Gamam_P2.*conj(Gamam_P3);
      
    Gamam_P2 = abs(filter(ones(1,1*16), 1, argmax2, [], 2));
    
%     Gamam_P3 = filter(ones(1,2*16), 1, abs(Gamam_P3).^2,[], 2);
    
%     Gamam_P2 = Gamam_P2./Gamam_P3;
%     
%     Gamam_P2(:,1:2*16) = [];
...
%             Gamam_P2_temp=Gamam_P2_temp+real(argmax2(1,jj+ii-1));
%         end
...
% 	end
    ...
    for ii=1:length(Gamam_P2)
        if real(Gamam_P2(1,ii))>0.6
           ii
%            SNR
           break;    
       end
