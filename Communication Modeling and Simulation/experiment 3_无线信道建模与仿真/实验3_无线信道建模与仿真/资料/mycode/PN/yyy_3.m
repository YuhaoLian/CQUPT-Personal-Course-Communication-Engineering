...
% 	for nframe=1:FrameNum
        ...
        TxBits = randint(1,Info_length*2,2);		
		ModData = data_modulate(TxBits, 'QPSK');
        OfdmSymbol_Data = sqrt(Info_length) * ifft(ModData,Info_length);
        
        ChannelInput=[OfdmSymbol_Data ChannelInput OfdmSymbol_Data];
		...
		ChannelOutput2=ChannelOutput(1,1+Info_length:end);         
        ChannelOutput3=ChannelOutput(1,1:end-Info_length);  
		for ii=1:M*Info_length;
            Gamam_P_temp=0;
            P_temp=0;
			for jj=1:length(PN_code)
                Gamam_P_temp=Gamam_P_temp+ChannelOutput3(1,jj+ii-1)*conj(ChannelOutput2(1,jj+ii-1));
                P_temp=P_temp+ChannelOutput2(1,jj+ii-1)*conj(ChannelOutput2(1,jj+ii-1));
			end
            Gamam_P(1,ii)=((abs(Gamam_P_temp))^2)/(P_temp^2);
		end
...
%             Gamam_P_temp=0;
% 			for jj=1:length(PN_code)
%                 Gamam_P_temp=Gamam_P_temp+ChannelOutput2(1,jj+ii-1)*conj(ChannelOutput2(1,jj+ii-1));
% 			end
