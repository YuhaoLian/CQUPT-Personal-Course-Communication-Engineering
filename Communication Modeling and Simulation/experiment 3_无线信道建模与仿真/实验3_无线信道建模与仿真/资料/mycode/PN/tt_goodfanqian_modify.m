...
for SNR=0:1:10
	...
	for nframe=1:FrameNum
        ...
        ChannelOutput=Standard_Multipath11(ChannelOutput,fc,V,fs);  
        
        %Remove sampls
        Step_cz=Insert_samples;
        for i=1:SubCarrNum*SymbolNum
            ChannelOutput_temp(1,i)=ChannelOutput(1,Insert_samples*(i-1)+Step_cz);
        end
        ChannelOutput=ChannelOutput_temp;
        
		ChannelOutput2=[zeros(1,SubCarrNum) ChannelOutput zeros(1,SubCarrNum)];        
		for ii=1:(SymbolNum+1)*SubCarrNum;
            Gamam_P_temp=0;
		...
		Gamam_P=Gamam_P(1,1:1:(end-SubCarrNum+1));
		argmax=abs(Gamam_P);
		       

		argMax=max(argmax(1,(SubCarrNum*SymbolNum-10):1:(SubCarrNum*SymbolNum+10)));
        for jj=(SubCarrNum*SymbolNum-10):1:(SubCarrNum*SymbolNum+10)
            if argmax(1,jj)==argMax 
               angle_offset=angle(Gamam_P(1,jj))*SymbolNum/(2*pi)-frequency_offset;
               time_offset=jj-SubCarrNum*SymbolNum;
...
subplot(312);
plot(1:length(argmax)-SubCarrNum,argmax(1,SubCarrNum+1:end));
grid on;

subplot(313);
for m = 2:SymbolNum
    plot((-SubCarrNum/2)+1:1:SubCarrNum/2,argmax(1,m*SubCarrNum-(SubCarrNum/2)+1:(m+1)*SubCarrNum-(SubCarrNum/2)));  
    hold on
end
grid on;
