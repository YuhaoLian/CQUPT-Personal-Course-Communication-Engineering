...
PN_code=PN_Signal(2*Info_length);
PN_code=PN_code(1,(Info_length+1):1:end);
	...
	for nframe=1:FrameNum
		...
		Noise_Var = E/(2*snr);
       	for m = 1:1:M
            ...
            %Pass through time dispersive channel
%             OfdmSymbol_Pilot=Standard_Multipath1(OfdmSymbol_Pilot,fc,V,fs);    
		...
		argMax=max(argmax(1,(Info_length*M-Info_length+1):1:(Info_length*M+Info_length-1)));
        for jj=(Info_length*M-Info_length+1):1:(Info_length*M+Info_length-1)
