...
% % 	TDLCoef(:,kk-TimeBegin)=ones(length(PowerDelayProfile),1);%sqrt(1/N0)*sum(cos(Wm*cos(Ank)*(kk+1e8)*Ts+Phase(1))+j*sin(Wm*sin(Ank)*(kk+1e8)*Ts+Phase(2))).';
% % end
% 
% TDLCoef=ones(length(PowerDelayProfile),length(TimeSignal));
% 
...
OutSignal=TimeSignal+[0 TimeSignal(1,1:end-1)];
