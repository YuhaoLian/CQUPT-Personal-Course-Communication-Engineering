...
fs = 20+6; 		%System bandwith is 5MHz
V=0;
FrameNum=10;
...
for  mmmm=1:1:length(Mf_container);
    
    Mf=Mf_container(1,mmmm)
    clear('M_sinal');
    clear('Reference_sinal');
    clear('PilotCarrNum');
    clear('PilotIndex');
    clear('DataIndex');
	...
	alpha=(SubCarrNum-PilotCarrNum)/SubCarrNum;
    ...
    kkkk=0;
	for SNR_number=1:2:10
        kkkk=kkkk+1
        SNR=SNR_number;
		Rho=(alpha*SNR)/((SNR*alpha)+1);
		snr=10^(SNR/10);
		E=1;                % 符号能量假设为单位能量.
...
%         ChannelOutput=Standard_Multipath1(ChannelOutput,fc,V,fs);
		...
		Error_ratio_time(1,kkkk)=error_ratio_time/(FrameNum-2);
	...
	figure(1)
	semilogy(1:2:10,Error_ratio_time,picture_style);
	grid on;
	hold on;
