...
%% 正交解调生成解调信号
rt = FM_Demodulation_QI(sft,t,fc,fm,1/dt);
...
%% 绘制已调信号时域图和信噪比条件下含20dB高斯白噪声的已调信号时域图
A = 1;
SNRdB = 20;
SNR = 10.^(SNRdB/10);
sigma = A*A./SNR ;
...
out_sft = noise + sft;
rt = FM_Demodulation_QI(out_sft,t,fc,fm,1/dt);
recieve_signal_20dB = rt;
figure
subplot(3,1,1);plot(t,mt);		           %绘制调制信号的时域图
xlabel('时间t');
title('调制信号的时域图');
subplot(3,1,2);plot(t,out_sft);		       %绘制已调信号的时域图
xlabel('时间t');
title('20dB噪声条件下已调信号的时域图');
subplot(3,1,3);plot(t,recieve_signal_20dB);		       %绘制已调信号的时域图
xlabel('时间t');
title('20dB噪声条件下解调信号的时域图');


%% 绘制已调信号时域图和信噪比条件下含40dB高斯白噪声的已调信号时域图
A = 1;
SNRdB = 40;
SNR = 10.^(SNRdB/10);
sigma = A*A./SNR ;
...
subplot(3,1,2);plot(t,out_sft);		       %绘制已调信号的时域图
xlabel('时间t');
title('40dB噪声条件下已调信号的时域图');
subplot(3,1,3);plot(t,recieve_signal_40dB);		       %绘制已调信号的时域图
xlabel('时间t');
title('40dB噪声条件下解调信号的时域图');
